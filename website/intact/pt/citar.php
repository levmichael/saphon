<?php include("../saphon.php"); ?>
<?php include("header-title.php"); ?>
<?php include("nav-about.php"); ?>

<div id="content">
<h5>Citar</h5>

Para citar este sítio web, ou um grupo de dados contidos nele, recomendamos a seguinte citação (favor adicionar o número de versão correspondente):

<blockquote>
Michael, Lev, Tammy Stark, and Will Chang (compilers). 2012. South American Phonological Inventory Database v<?=$version?>. Survey of California and Other Indian Languages digital resource. Berkeley: University of California.
</blockquote>

Para citar um inventário fonológico em particular, recomendamos a seguinte citação:

<blockquote>
Michael, Lev, Tammy Stark, and Will Chang (compilers). 2012. Phonological inventory of Yaminahua. In South American Phonological Inventory Database v<?=$version?>. Survey of California and Other Indian Languages digital resource. Berkeley: University of California.
</blockquote>

Se você consultar a fonte original na qual baseamos o inventário apresentado neste sítio web (as fontes originais estão indicadas para cada inventário) e usar o inventário naquela fonte original, deverá citar a fonte original.
</p>

</div>		
</div>

</body>
</html>
