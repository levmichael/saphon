...
* `processdetails`: A list of details pertaining to all of the (nasal) processes active in the language. Each process described in this list must also refer to a process in the `phoneme` list one or more times. Each value in this list is a dict. The dict values of this list must not have repeated values of the conjunction of their `proc_type` and `proc_name` values (see below).
  * `proc_name`: The name of the process described. The value must match a string in the `proc_names` list in the `phonemes` list. This value is a string.
  * `proc_type`: The type of process described. The value is a string that must match one of the values of ... (TODO: add pointer to controlled vocabulary for this field). (TODO: check that this value matches the prefix of `proc_name`.
